...
func _physics_process(delta):
	...
	if bodies:
		...
		if atlas > -1:
			if tile == Vector2(0, 0):
				$SoundPellet.play()
				ScoreKeeping.P2_Points += 1
			elif tile == Vector2(1, 0):
				$SoundStrawberry.play()
				ScoreKeeping.P2_Points += 3
				loaded = true

	var P2score = get_parent().get_node("LevelGUI/Player2Score")
	P2score.text = str(ScoreKeeping.P2_Points)
	...
	if loaded:
		get_parent().get_node("LevelGUI/Player2Indicator").color = Color(0.94902, 0.243137, 0.243137)
	else:
		get_parent().get_node("LevelGUI/Player2Indicator").color = Color(0, 0, 0)
